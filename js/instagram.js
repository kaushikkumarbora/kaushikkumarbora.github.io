var Instagram = (function(){
	...
	var render = function(data){
		for(var em in data){
			...
			$('<section class="archives album"><h1 class="year">'+data[em].year+'<em>'+data[em].month+'month</em></h1>\
	...
	var ctrler = function(data){
		...
		for(var i=0,len=data.length;i<len;i++){
			...
			var text = data[i].caption ? data[i].caption.text : ''; // data[i].caption It is possible for null
			var key = y+"-"+m;
			if(imgObj[key]){
	...
	var getList = function(url){
		$(".open-ins").html("Pictures from instagram，Loading…");
		$.ajax({
			url: url,
			type:"GET",
			dataType:"jsonp",
			success:function(re){
				if(re.meta.code == 200){
					...
					if(next){
						...
						$(".open-ins").html("Pictures from instagram，Click here to visit");
						ctrler(_collection);
					}
				}else{
